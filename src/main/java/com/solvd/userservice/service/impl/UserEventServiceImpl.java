...
public class UserEventServiceImpl implements UserEventService {
    ...
    public Mono<Void> update(User user) {
        return checkIfEmailIsAvailable(user)
                ...
                .flatMap(u -> {
                    AbstractEvent userUpdateEvent = new UserUpdateEvent();
                    userUpdateEvent.setType(EventType.USER_UPDATE);
                    userUpdateEvent.setPayload(user);
                    userUpdateEvent.setAggregateId(user.getId());
                    eventRepository.save(userUpdateEvent);
                    return messageSender.sendMessage("events", 0, String.valueOf(user.hashCode()), userUpdateEvent)
                            .then();
                });
    }
    ...
    public Mono<Void> updatePassword(String userId, String newPassword) {
        AbstractEvent resetPasswordEvent = new ResetPasswordEvent();
        resetPasswordEvent.setAggregateId(userId);
        resetPasswordEvent.setPayload(passwordEncoder.encode(newPassword));
        resetPasswordEvent.setType(EventType.RESET_PASSWORD);
        eventRepository.save(resetPasswordEvent);
        return messageSender.sendMessage("events", 0, userId, resetPasswordEvent)
                .then();
    }
